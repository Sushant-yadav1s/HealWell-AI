...
export default function DietPlanner() {
  ...
  try {
    ...
    // 🔥 DIRECT BACKEND CALL
    const res = await fetch(`http://localhost:5000/weekly-planner/${plannerId}`, {
      headers: {
        "Authorization": `Bearer ${token}`
      }
    });

    const plannerResponse = await res.json();
    ...
    toast.error('Failed to load weekly planner');
    navigate('/dashboard');
  } finally {
    setLoading(false);
  }
...
const generatePlanFromReport = async () => {
  ...
  try {
    ...
    // 🔥 1. FETCH REPORT FROM BACKEND
    const reportRes = await fetch(`http://localhost:5000/reports/${reportId}`, {
      headers: {
        "Authorization": `Bearer ${token}`
      }
    });
    ...
    try {
      const plannerRes = await fetch("http://localhost:5000/weekly-planner/generate", {
        method: "POST",
        headers: {
      ...
      const plannerResponse = await plannerRes.json();

      if (plannerResponse.success && plannerResponse.data) {
        const backendPlanner = plannerResponse.data;
        ...
        const transformedPlan: WeeklyPlan = {
          ...
          weekPlan: backendPlanner.weekPlan || []
        };
