...
export default function Dashboard() {
  ...
  try {
    ...
    const reportsRes = await fetch(
      `http://localhost:5000/reports/user/${user.id}`,
      {
        headers: {
    ...
    // 🔥 2. FETCH AVERAGE SCORE
    const scoreRes = await fetch(
      `http://localhost:5000/reports/average-score/${user.id}`,
      {
        headers: {
          "Authorization": `Bearer ${token}`
        }
    ...
    if (scoreData.success) {
      setAvgHealthScore(scoreData.averageScore || 0);
    }

    // 🔥 WEEKLY PLANS (keep your existing code)
    try {
    ...
    try {
      ...
      if (allRemindersResponse.success && allRemindersResponse.data) {
        ...
        const now = new Date();
        const reminders = allRemindersResponse.data
          .filter((r: any) => r.isActive && new Date(r.nextReminder) > now)
          .slice(0, 5);
